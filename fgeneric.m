function Fvalue = fgeneric(x, FUNC_ID, DIM, itrial, repeat, dataPath, PARAMS)
...
persistent actFunc Fopt isRowFormat LastEval BestFEval lastWriteEval Ftarget;  % 已修改
...
if isempty(DefaultPARAMS)
    ...
    DefaultPARAMS = struct('algName', 'not-specified', 'comments', '', ...
        'inputFormat', 'col', 'filePrefix', 'exp');
    maxFunEvalsFactor = 1e6;
end
if isempty(handlesF)
    handlesF = benchmarks('handles');
    %     if exist('benchmarksnoisy', 'file') % 已注释
...
if ischar(x)
    ...
    elseif strcmpi(x, 'initialize')
        ...
        PARAMS.runCounter = 1;
        PARAMS.dataFilePrefix = fullfile(dataPath, PARAMS.filePrefix);  % 已修改 需注意
        
        PARAMS.dataPath = dataPath;  % 新加的
    ...
    elseif strcmpi(x, 'repeat')
        indexFilePrefix = fullfile(dataPath, CurrentPARAMS.filePrefix);    %% 已修改
        CurrentPARAMS.indexFile = sprintf('%s_f%d.info', indexFilePrefix, CurrentPARAMS.funcId);
        CurrentPARAMS.dataFilePrefix = fullfile(dataPath, CurrentPARAMS.filePrefix); % 文件夹名需要
        CurrentPARAMS.repeat = repeat;
        
        if ~isfield(PreviousPARAMS,'repeat')
            PreviousPARAMS.repeat = CurrentPARAMS.repeat;
        end
        if ~isfield(PreviousPARAMS,'dataFilePrefix')
            PreviousPARAMS.dataFilePrefix = CurrentPARAMS.dataFilePrefix; % 文件夹名需要
            PreviousPARAMS.indexFile = CurrentPARAMS.indexFile;
        end                
        if ~strcmp(CurrentPARAMS.indexFile,PreviousPARAMS.indexFile)
            [indexFileId, msg] = fopen(CurrentPARAMS.indexFile,'w');
            try
                fprintf(indexFileId, ':%d,%.1e\n', ...
                    LastEval.num, BestFEval.F - Fopt - CurrentPARAMS.precision);
            catch
            end
            fclose(indexFileId);
        end
        ...
        writeDataHeader(PARAMS.dataFile, Ftarget);
        
        LastEval = struct('num', 0, 'F', inf, 'Fnoisy', inf, 'bestFnoisy',...
            inf, 'x', []); % minimization % 重新初始化evaluations
        lastWriteEval = 0;
        BestFEval = LastEval; % 重新初始化fbest
        BestFEval.isWritten = 1;
        
        idxFTrigger = LastEval.F;
        fTrigger = 10^(idxFTrigger/nbPtsF);
        idxEvalsTrigger = 0;
        evalsTrigger = floor(10^(idxEvalsTrigger/nbPtsEvals)); % nbPtsEvals=20,
        idxDIMEvalsTrigger = 0;
        
        return;        
                
    elseif strcmpi(x, 'repeatfinal')
        if isempty(actFunc)
            warning('FGENERIC:initfinal','%s', ...
            ...
            if isfield(CurrentPARAMS,'dimension')
                writeFinalData(CurrentPARAMS,BestFEval,lastWriteEval, ...
                    LastEval,Ftarget); %% 已修改 原Fopt, 所有维度
    ...
    elseif strcmpi(x, 'restart')
        ...
        rbuffer = [rbuffer sprintData(LastEval.num, LastEval.F, BestFEval.F, ...
            LastEval.Fnoisy, LastEval.bestFnoisy, LastEval.x,Ftarget)]; %% 已修改 原Fopt, 所有维度
        ...
        if ~isempty(rbuffer)
            ...
            if rdataFileId < 0
                ...
                fprintf(rdataFileId,'%% repeat: %s\n', FUNC_ID); % 已修改，原restart
...
if ~isfield(CurrentPARAMS,'dimension') || CurrentPARAMS.repeat ~= PreviousPARAMS.repeat
    CurrentPARAMS.dimension = DIM;
    if isParamsMatching && DIM == PreviousPARAMS.dimension
        CurrentPARAMS = setNextDataFile(CurrentPARAMS, 1);
        writeDataHeader(CurrentPARAMS.dataFile, Ftarget);
    ...
    for j = 1:POPSI
        ...
        if (evalsj >= evalsTrigger)
            ...
            buffer = [buffer sprintData(evalsj, Ftrue(j), BestFEval.F, ...
                Fvalue(j), LastEval.bestFnoisy, x(1:2,j),Ftarget)]; %% 已修改 原Fopt, 所有维度
...
if dataFileId < 0
    ...
    fprintf(dataFileId, ...
        ['%% #fes | fit - Ftarget (%13.12e) ' ...
        '| best fit - Ftarget | measured fit | best ' ...
        'measured fit | x1 | x2...\n'], Fopt);
    fclose(dataFileId);
end
...
if dataFileId < 0
    ...
    if ~BestFEval.isWritten
        if BestFEval.num > lastWriteEval
            ...
            fprintf(dataFileId,'%s', ...
                ...
                BestFEval.Fnoisy,BestFEval.bestFnoisy,LastEval.x(1:2,:),Fopt));  % 已修改
        else
            fclose(dataFileId);
    ...
    if LastEval.num > lastWriteEval
        fprintf(dataFileId,'%s', ...
            ...
            LastEval.Fnoisy, LastEval.bestFnoisy,LastEval.x(1:2,:),Fopt)); % 已修改
    end
    fclose(dataFileId);
end % if dataFileId > -1
...
if dataFileId < 0
    ...
    for j = 1:length(lines)
        ...
        if sscanf(lines{j},'%d',1) > BestFEval.num
            fprintf(dataFileId,'%s', ...
                sprintData(BestFEval.num,BestFEval.F,BestFEval.F, ...
                BestFEval.Fnoisy, BestFEval.bestFnoisy,BestFEval.x(1:2,:),Fopt)); %% 已修改 原Fopt, 所有维度
            break;
        end
...
dataFile = sprintf('%s_%02u_%02u_f%d_DIM%d.tdat', ...
    PARAMS.dataFilePrefix, ...
    PARAMS.itrial, ...
    PARAMS.repeat, ...
    PARAMS.funcId, ...
    PARAMS.dimension);
hdataFile = sprintf('%s_%02u_%02u_f%d_DIM%d.dat', ...
    PARAMS.dataFilePrefix, ...
    PARAMS.itrial, ...
    PARAMS.repeat, ...
    PARAMS.funcId, ...
    PARAMS.dimension);
rdataFile = sprintf('%s_%02u_%02u_f%d_DIM%d.rdat', ...
    PARAMS.dataFilePrefix, ...
    PARAMS.itrial, ...
    PARAMS.repeat, ...
    PARAMS.funcId, ...
